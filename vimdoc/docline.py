...
class Section(BlockDirective):
  ...
  def __init__(self, args):
    # Used to collect children at Module processing time.
    self.parent_id = None
    super(Section, self).__init__(args)

  def Assign(self, name, ident, parent):
    ...
    # parent and whose names don't match \w+
    if parent is not None:
      self.parent_id = parent.lower()

  def Update(self, block):
    block.SetType(vimdoc.SECTION)
    block.Local(name=self.name, id=self.id, parent_id=self.parent_id)
